...
export function TransactionWorkspace({ data, scope, actions, canReadReports, onOpenInvoice, onOpenExpense, renderAccountActions, renderTransactionActions }: {
  ...
  return <section aria-label="Transactions" className="flex min-w-0 flex-col gap-4 px-4 py-4 sm:px-6">
    ...
    <div className="overflow-x-auto rounded-lg border"><table className="w-full min-w-[960px] text-left text-[13px] leading-5"><thead className="bg-[var(--table-header-bg)]"><tr>{["Date", "Transaction", "Type", "Unit / tenant", "Status", "Amount", "Actions"].map(label => <th key={label} className={`px-3 py-2 text-[11px] font-medium text-muted-foreground ${label === "Amount" || label === "Actions" ? "text-right" : ""}`}>{label}</th>)}</tr></thead><tbody>
