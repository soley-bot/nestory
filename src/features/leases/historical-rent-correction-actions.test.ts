...
describe("historical rent correction actions", () => {
  ...
  it("retains a blocked settled decrease as an error without refreshing financial evidence", async () => {
    rpc.mockResolvedValue({ data: null, error: {
      message: "historical_rent_correction_blocked",
      details: '[{"code":"historical_rent_tenant_credit_unsupported"}]',
    } });
    const form = correctionForm();
    form.set("previewHash", "a".repeat(64));
    await expect(applyHistoricalRentCorrectionAction({}, form)).resolves.toMatchObject({
      status: "error", message: expect.stringContaining("no refund has been recorded"),
    });
