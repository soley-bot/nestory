...
function historicalRentBlockerLabel(code: string) {
  if (code === "historical_rent_dependent_owner_cash") {
    return "Collected rent has funded another owner charge or distribution. Reverse that dependent transaction through its correction workflow, then preview again.";
  }
  if (code === "historical_rent_owner_roster_changed") {
    return "Owner shares have changed since collection. Resolve the ownership handoff before replaying this period.";
  }
  if (code === "historical_rent_tenant_credit_unsupported") {
    return "Rent cannot be reduced below money already collected until tenant-credit accounting is available. No refund has been recorded.";
  }
  if (code === "historical_rent_payment_account_unavailable") {
    return "Restore the original payment account or resolve its missing account history, then preview again.";
  }
