...
export const DEFAULT_RENT_INCOME_PAGE_SIZE = 50;

type RentIncomeSearchParams = Record<string, SearchParamValue>;

export function parseRentIncomeSearchParams(
  params: RentIncomeSearchParams,
...
export function getRentIncomeMonthScope(month: string) {
...
function parsePageSize(value: SearchParamValue) {
  const candidate = getPositiveIntegerSearchParam(
    value,
    DEFAULT_RENT_INCOME_PAGE_SIZE,
  );

  return RENT_INCOME_PAGE_SIZE_OPTIONS.includes(
    candidate as (typeof RENT_INCOME_PAGE_SIZE_OPTIONS)[number],
  )
    ? candidate
    : DEFAULT_RENT_INCOME_PAGE_SIZE;
}

function parseStatus(value: SearchParamValue): RentIncomeStatusFilter {
  const candidate = getFirstSearchParam(value);

  return candidate === "open" ||
    candidate === "partially_received" ||
    candidate === "received" ||
    candidate === "posted"
    ? candidate
    : "all";
