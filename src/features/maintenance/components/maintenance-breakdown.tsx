        ...
        {visibleCategoryStats.length > 0 ? (
          <div className="overflow-hidden rounded-md border border-border bg-background/35">
            ...
            {visibleCategoryStats.map((category) => {
              ...
              return (
                <Link
                  className="grid grid-cols-[minmax(0,1fr)_68px_80px_54px] items-center border-b border-border px-3 py-3 text-sm last:border-b-0 hover:bg-muted"
        ...
        {visiblePropertyStats.length > 0 ? (
          <div className="space-y-1">
            {visiblePropertyStats.map((property) => {
              ...
              return (
                <Link
                  className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-border py-2.5 text-sm last:border-b-0 hover:bg-muted"
                  href={getMaintenanceListHref(viewQuery, {
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            <div className="mt-2 text-sm text-muted-foreground">Active maintenance cases</div>
          </div>
          ...
          <div className="space-y-1.5">
            {visibleCategoryStats.length > 0 ? (
              visibleCategoryStats.slice(0, 3).map((category) => (
                <Link
                  className="block rounded-md border border-border bg-background/35 p-2.5 hover:bg-muted"
                  href={getMaintenanceListHref(viewQuery, {
                    query: category.category,
                    review: "open",
                  })}
...
function WidgetListRow({ helper, href, label, tone, value }: WidgetListRowProps) {
  return (
    <Link
      className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-md border border-border bg-background/35 p-3 text-sm hover:bg-muted"
      href={href}
    >
      <span className="min-w-0">
...
function MetricTile({
  ...
  return (
    <div className={cn("rounded-md border p-3", toneSoftClass(tone))}>
      ...
      <div className="mt-1 text-xs font-medium uppercase tracking-[0.04em] text-muted-foreground">
        {label}
      </div>
    </div>
  );
