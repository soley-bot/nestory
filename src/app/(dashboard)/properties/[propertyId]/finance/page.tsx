...
export default async function PropertyFinancePage({
  ...
  const allData = await getFinanceOperationsData(context.organizationId, propertyId, { includeExpenses: query.view !== "rent", completeTransactionHistory: !["rent", "expenses"].includes(query.view ?? ""), includeAccountSources: !["rent", "expenses", "owner"].includes(query.view ?? "") || query.view === "owner" });
