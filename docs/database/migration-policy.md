...
Production migrations run only from the exact merged `main` SHA in the protected `production-database` GitHub environment. The job is serialized, verifies that the hosted ledger is an exact Git prefix before `db push`, and asks Postgres to SHA-256 every hosted row using a length-prefixed version, name, and ordered statement-descriptor representation. The query returns only identities, counts, byte lengths, and hashes; hosted SQL is never transported through the runner. The verifier reconstructs every ordinary hosted statement descriptor from the corresponding Git migration and requires exact canonical-hash equality. Supabase-omitted statement delimiters and newline encoding are normalized; SQL tokens are not. The job requires exact equality, linked lint, and a no-op dry-run after the push. The `Vercel - nestory: Database` status reports this hosted result.

The immutable 2026-08-21 canonical-hash manifest pins all 97 reconciled rows and must remain an exact prefix of both Git and hosted history. Six pre-guardrail rows have incomplete or non-reconstructing hosted statement payloads. They are separately declared under `hostedContentExceptions`, with the exact Git SQL hash, historical hosted evidence hash, database-side hosted hash, and canonical descriptor hash pinned across the reconciliation manifests. This acknowledges historical ledger-content divergence; it does not claim SQL equivalence, permit either side to change, or authorize a hosted repair. Every ordinary baseline row and every future migration must reconstruct exactly from the live hosted descriptors. A one-byte SQL change produces a different descriptor and canonical hash and fails the gate.
