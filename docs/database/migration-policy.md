...
Production migrations run only from the exact merged `main` SHA in the protected `production-database` GitHub environment. The job is serialized, verifies that the hosted ledger is an exact Git prefix before `db push`, and compares every hosted migration name and recorded statement payload with the corresponding Git migration. Supabase-omitted statement delimiters and newline encoding are normalized; SQL tokens are not. The job requires exact equality, linked lint, and a no-op dry-run after the push. The `Vercel - nestory: Database` status reports this hosted result.

Six pre-guardrail rows have incomplete or non-reconstructing hosted statement payloads. They are declared under `hostedContentExceptions` in the immutable 2026-08-21 reconciliation manifest. Each declaration pins the exact Git SQL hash and exact hosted `{version,name,statements}` hash. A second immutable manifest pins a deterministic database-side SHA-256 over the same normalized hosted identity and payload, allowing large legacy rows to be verified without transferring their SQL through the runner. It acknowledges a historical ledger-content divergence; it does not claim SQL equivalence, permit either side to change, or authorize a hosted repair. Every undeclared and every future migration is fetched in bounded pages and must reconstruct exactly from the hosted statements.
