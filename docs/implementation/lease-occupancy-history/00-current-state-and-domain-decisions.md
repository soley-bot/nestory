...
| Occupant | An individual whose accepted occupancy-participant version is in `present`/`ended` lifecycle, every query-material participant and accepted actual-occupancy boundary is `known` or an end is `open_current` through `as_of_date`, and whose participant interval is contained by that actual occupancy. Planned, cancelled, or unknown-boundary participation is not confirmed residence. A tenant or authorized-occupant role alone is association or authorization, not proof of physical presence. |
...
| Unit during an effective tenancy | An `accepted` `lease_occupancies` version in `occupied`, `notice_given`, or `vacated` lifecycle supplies the exact Lease/Unit association when its typed interval has `definite_overlap` | `leases.unit_id` becomes a protected current projection during migration. `reserved`/planned, `cancelled_before_effective`, `superseded`, `voided`, and `legacy_unresolved` rows remain visible only as scheduled or non-authoritative evidence. |
| Tenant responsibility | `accepted` `lease_parties` versions in `primary_tenant` or `co_tenant` roles whose lifecycle is `effective` or `ended`, classified over their typed intervals | `primary_tenant_person_id` and `tenant_name` become protected projections. `planned`, `cancelled_before_effective`, `superseded`, `voided`, and `legacy_unresolved` versions cannot define effective responsibility. |
...
| Lease/Unit physical use | An `accepted` `lease_occupancies` version in an `occupied`, `notice_given`, or `vacated` lifecycle whose confirmed actual boundaries produce `definite_overlap` for the queried interval | Lease header dates, scheduled-only evidence, `cancelled_before_effective`, `superseded`, `voided`, `legacy_unresolved`, and unknown-boundary evidence are not confirmed physical use. |
| Person-level physical occupancy | An `accepted` occupancy-participant version in a `present` or `ended` lifecycle linking an exact individual/party to an `accepted`, non-cancelled actual occupancy, with every participant and occupancy boundary material to the query confirmed or resolved as `open_current` through `as_of_date` | Planned, `cancelled_before_effective`, superseded, voided, `legacy_unresolved`, or unknown-boundary participation remains scheduled, associated, authorized, or unknown—not confirmed residence. |
...
| Does a tenant/authorized-occupant role plus Lease occupancy prove that Person physically lived there? | No. It proves association or authorization during the Lease/Unit occupancy. Confirmed person-level physical occupancy requires an accepted `present`/`ended` participant whose interval is contained by accepted actual occupancy, with every query-material participant and occupancy boundary `known` or an end resolved `open_current` through `as_of_date`. |
