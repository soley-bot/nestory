...
5. When the exact-main workflow contains an explicitly approved one-time recovery artifact, classify the hosted checkpoint here before the dry-run. Run the artifact only at an exact reviewed starting or resumable ledger; after every recovery migration is present, verify completed state and skip it. Unknown states fail closed. Each artifact must pin its complete target and metadata contract and is not a migration-history repair or a general exception path.
...
The custom-role release has one source-controlled recovery step for twenty-one legacy hosted functions whose stored bodies used CRLF while checked forward-migration replacements expect LF. Eight approved targets were already normalized. The final thirteen approved targets—nine required by `20260822053215` and four required by `20260822071638`—were normalized together at hosted ledger 103. `scripts/classify-hosted-function-newline-recovery.sql` permits normalization only at count 103 and head `20260822045638`; it also recognizes the reviewed 105/106 partial release checkpoints after predecessor migrations have replaced their expected functions. `scripts/normalize-hosted-function-newlines.sql` pins every exact raw and normalized SHA-256 definition, replaces CRLF with LF only, and verifies unchanged function OID, owner, ACL, execution settings, and planner metadata. Missing, duplicate, unknown, or semantically different targets abort the transaction. The step does not alter migration files or migration history and becomes a verified no-op for later releases.

### Approved financial evidence branch backfill recovery

The first protected apply recorded `20260822053215` and `20260822061424`, then `20260822071638` rolled back when its `documents.branch_id` backfill reached three referenced financial-evidence documents protected by the released strict immutability trigger. The approved recovery is limited to those three exact document identities, their pinned non-branch row and financial-reference hashes, and their one exact active branch.

At the exact 105-row checkpoint, the workflow captures a preservation snapshot, verifies the four remaining normalized predecessor functions, and temporarily replaces only `app_private.guard_financial_evidence_document()` with a hash-pinned definition. Its sole additional allowance requires an exact approved document ID, organization, locked-reference state, null old branch, exact approved new branch, and equality of every JSON field except `branch_id`. Every other update and delete keeps the released strict behavior. Function and trigger OIDs, owner, ACL, comments, execution configuration, and planner metadata must match their pinned values before and after replacement.

The migration apply runs with an exit trap that restores the exact strict definition whether apply succeeds or fails. The postflight requires the 107-row ledger, all three exact branch assignments, the strict guard hash and metadata, and a byte-identical preservation snapshot for document contents, financial references, related history, and Storage-object counts. Exact 106-row resume states are accepted only after the strict guard is restored; any other partial or changed state fails closed. No released migration or migration-history row is changed.
