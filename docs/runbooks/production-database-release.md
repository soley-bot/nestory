...
The custom-role release has one source-controlled recovery step for eight legacy hosted functions whose stored bodies used CRLF while the forward migration's checked replacement expects LF. Five approved targets are already normalized. The three approved photo targets must be either all at their exact raw hashes or all at their exact LF-normalized hashes; mixed or partial states fail before any definition executes. `scripts/classify-hosted-function-newline-recovery.sql` permits normalization only at hosted ledger count 103 and head `20260822045638`, reports completion once all four package migrations exist, and rejects partial release states. `scripts/normalize-hosted-function-newlines.sql` pins each exact raw and normalized SHA-256 definition, replaces CRLF with LF only for the three photo targets, and verifies the normalized hashes plus unchanged function OID, owner, ACL, execution settings, and planner metadata. Missing, duplicate, unknown, or semantically different targets abort the transaction. The step does not alter migration files or migration history and becomes a verified no-op for later releases.
