...
-- Integrated from PR #112 exact 73bed403da2ec4a8b0cad7042d58463211534ca6.
-- Source proposal: 20260901005951_paid_expense_transactions.sql (not replayed).
-- Reconciled after accepted candidate 2cfebd6fbcbb99e61531fb0a3efb3d39ecc22de0.
CREATE TABLE public.expense_transactions (
  ...
  FOREIGN KEY (organization_id, pay_from_account_id)
...
CREATE TABLE public.expense_transaction_lines (
  ...
  FOREIGN KEY (organization_id, submission_id)
    ...
    ON DELETE RESTRICT,
  FOREIGN KEY (organization_id, category_account_id)
    REFERENCES public.finance_accounts(organization_id, id) ON DELETE RESTRICT,
...
-- A private capability binds nested child mutations to actor, SQL transaction,
-- parent, and operation. Client-controlled GUC values alone grant no authority.
CREATE TABLE app_private.expense_transaction_capability (
  singleton boolean PRIMARY KEY DEFAULT true CHECK (singleton),
  secret text NOT NULL
);
INSERT INTO app_private.expense_transaction_capability(singleton, secret)
VALUES (true, encode(extensions.digest(gen_random_uuid()::text || clock_timestamp()::text, 'sha256'), 'hex'));
REVOKE ALL ON TABLE app_private.expense_transaction_capability FROM PUBLIC, anon, authenticated, service_role;

CREATE FUNCTION app_private.set_expense_transaction_context(
  p_organization_id uuid, p_transaction_id uuid, p_operation text, p_enabled boolean
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE v_token text;
BEGIN
  IF p_enabled THEN
    IF (SELECT auth.uid()) IS NULL OR p_operation NOT IN ('submit', 'review', 'reverse') THEN
      RAISE EXCEPTION 'Expense transaction authority is invalid' USING ERRCODE = '42501';
    END IF;
    SELECT encode(extensions.digest(concat_ws(':', secret, p_organization_id,
      p_transaction_id, p_operation, auth.uid(), pg_catalog.txid_current()), 'sha256'), 'hex')
    INTO STRICT v_token FROM app_private.expense_transaction_capability WHERE singleton;
    PERFORM set_config('app.expense_transaction_id', p_transaction_id::text, true);
    PERFORM set_config('app.expense_transaction_org', p_organization_id::text, true);
    PERFORM set_config('app.expense_transaction_operation', p_operation, true);
    PERFORM set_config('app.expense_transaction_token', v_token, true);
  ELSE
    PERFORM set_config('app.expense_transaction_id', '', true);
    PERFORM set_config('app.expense_transaction_org', '', true);
    PERFORM set_config('app.expense_transaction_operation', '', true);
    PERFORM set_config('app.expense_transaction_token', '', true);
    PERFORM set_config('app.expense_owner_cash_mode', '', true);
    PERFORM set_config('app.expense_line_description', '', true);
  END IF;
END;
$$;

CREATE FUNCTION app_private.has_expense_transaction_context(
  p_organization_id uuid, p_transaction_id uuid, p_operation text
) RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  SELECT (SELECT auth.uid()) IS NOT NULL AND EXISTS (
    SELECT 1 FROM app_private.expense_transaction_capability AS capability
    WHERE capability.singleton
      AND current_setting('app.expense_transaction_org', true) = p_organization_id::text
      AND current_setting('app.expense_transaction_id', true) = p_transaction_id::text
      AND current_setting('app.expense_transaction_operation', true) IN ('submit', 'review', 'reverse')
      AND (p_operation IS NULL OR current_setting('app.expense_transaction_operation', true) = p_operation)
      AND current_setting('app.expense_transaction_token', true) =
        encode(extensions.digest(concat_ws(':', capability.secret, p_organization_id,
          p_transaction_id, current_setting('app.expense_transaction_operation', true),
          auth.uid(), pg_catalog.txid_current()), 'sha256'), 'hex')
  );
$$;

CREATE FUNCTION app_private.guard_expense_transaction_child_status()
...
BEGIN
  ...
  WHERE line.organization_id = NEW.organization_id AND line.submission_id = NEW.id;
  IF v_transaction_id IS NOT NULL AND NOT app_private.has_expense_transaction_context(
    NEW.organization_id, v_transaction_id,
    CASE WHEN NEW.status = 'reversed' THEN 'reverse' ELSE 'review' END
  ) THEN
...
END;
$$;
CREATE TRIGGER expense_transaction_child_status_authority
BEFORE UPDATE OF status ON public.expense_submissions
FOR EACH ROW EXECUTE FUNCTION app_private.guard_expense_transaction_child_status();

CREATE FUNCTION app_private.lock_expense_transaction_scopes(
  p_organization_id uuid, p_transaction_id uuid, p_effective_date date
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE v_scope record; v_owner record;
BEGIN
  -- Lock every property month first, then every owner lifecycle, then cash.
  -- This order is independent of user-entered line order and cash instructions.
  FOR v_scope IN
    SELECT scope.property_id, transaction.currency, p_effective_date AS effective_date
    FROM public.expense_transaction_scopes AS scope
    JOIN public.expense_transactions AS transaction
      ON transaction.organization_id = scope.organization_id AND transaction.id = scope.transaction_id
    WHERE scope.organization_id = p_organization_id AND scope.transaction_id = p_transaction_id
    ORDER BY scope.property_id
  LOOP
    PERFORM app_private.lock_open_property_financial_month(
      p_organization_id, v_scope.property_id, v_scope.currency, v_scope.effective_date
    );
  END LOOP;
  FOR v_owner IN
    SELECT DISTINCT invoice.property_id, invoice.owner_person_id, invoice.currency
    FROM public.owner_invoices AS invoice
    JOIN public.expense_transaction_scopes AS scope
      ON scope.organization_id = invoice.organization_id AND scope.property_id = invoice.property_id
    WHERE scope.organization_id = p_organization_id AND scope.transaction_id = p_transaction_id
      AND invoice.lifecycle = 'issued'
    ORDER BY invoice.property_id, invoice.owner_person_id, invoice.currency
  LOOP
    PERFORM app_private.lock_owner_balance_lifecycle(
      p_organization_id, v_owner.property_id, v_owner.owner_person_id, v_owner.currency
    );
  END LOOP;
  FOR v_scope IN
    SELECT scope.property_id FROM public.expense_transaction_scopes AS scope
    WHERE scope.organization_id = p_organization_id AND scope.transaction_id = p_transaction_id
    ORDER BY scope.property_id
  LOOP
    PERFORM pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(
      p_organization_id::text || ':' || v_scope.property_id::text || ':owner-cash', 0
    ));
  END LOOP;
END;
$$;

REVOKE ALL ON FUNCTION app_private.set_expense_transaction_context(uuid,uuid,text,boolean)
  FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION app_private.has_expense_transaction_context(uuid,uuid,text)
  FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION app_private.guard_expense_transaction_child_status()
  FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION app_private.lock_expense_transaction_scopes(uuid,uuid,date)
  FROM PUBLIC, anon, authenticated, service_role;
...
REVOKE ALL ON FUNCTION app_private.can_read_expense_transaction(uuid, uuid)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION app_private.can_read_expense_transaction(uuid, uuid)
  TO authenticated;
...
CREATE FUNCTION public.get_expense_transaction_child_links(
  p_organization_id uuid, p_submission_ids uuid[]
) RETURNS TABLE(submission_id uuid, transaction_id uuid)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = '' AS $$
BEGIN
  IF (SELECT auth.uid()) IS NULL THEN
    RAISE EXCEPTION 'Not authenticated' USING ERRCODE = '28000';
  END IF;
  IF p_organization_id IS NULL OR p_submission_ids IS NULL
    OR cardinality(p_submission_ids) > 1000 THEN
    RAISE EXCEPTION 'Expense child lookup is invalid' USING ERRCODE = '22023';
  END IF;
  RETURN QUERY
  SELECT line.submission_id, line.transaction_id
  FROM public.expense_transaction_lines AS line
  JOIN public.expense_submissions AS submission
    ON submission.organization_id = line.organization_id AND submission.id = line.submission_id
  WHERE line.organization_id = p_organization_id
    AND line.submission_id = ANY(p_submission_ids)
    AND app_private.can_read_finance_property(p_organization_id, submission.property_id)
  ORDER BY line.submission_id;
END;
$$;
REVOKE ALL ON FUNCTION public.get_expense_transaction_child_links(uuid,uuid[])
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_expense_transaction_child_links(uuid,uuid[])
  TO authenticated;
...
BEGIN
  IF coalesce(v_transaction_setting, '') ~
    ...
    IF app_private.has_expense_transaction_context(
      p_organization_id, v_transaction_setting::uuid, NULL
    ) THEN
...
BEGIN
  ...
  IF v_transaction_id IS NOT NULL
    AND current_setting('app.expense_owner_cash_mode', true) = 'explicit' THEN
    RETURN 0;
  END IF;
...
DECLARE
  ...
  v_description text;
  v_line_description text := CASE WHEN EXISTS (
  ...
  ) THEN nullif(btrim(current_setting('app.expense_line_description', true)), '') END;
BEGIN
  IF p_organization_id IS NULL
...
BEGIN
  ...
  PERFORM app_private.set_expense_transaction_context(
    p_organization_id, p_transaction_id, 'review', true
  );
  IF v_decision = 'approve' THEN
    PERFORM app_private.lock_expense_transaction_scopes(
      p_organization_id, p_transaction_id, v_transaction.expense_date
    );
  END IF;
  ...
  ELSE
    FOR v_line IN
      ...
      ORDER BY line.sort_order
    LOOP
      PERFORM public.review_expense_with_account(
        p_organization_id,
        v_line.submission_id,
        'reject',
        v_reason,
...
BEGIN
  ...
  PERFORM app_private.lock_expense_transaction_scopes(
    p_organization_id, p_transaction_id, p_reversal_date
  );
  PERFORM app_private.set_expense_transaction_context(
    p_organization_id, p_transaction_id, 'reverse', true
  );
  FOR v_line IN
  ...
  PERFORM app_private.set_expense_transaction_context(p_organization_id, p_transaction_id, 'reverse', false);
  UPDATE public.expense_transactions AS transaction
  SET status = 'reversed',
      reversed_at = now(),
