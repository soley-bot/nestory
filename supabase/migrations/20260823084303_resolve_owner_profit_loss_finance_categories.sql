DROP FUNCTION public.get_owner_profit_loss_events_page(
  uuid, uuid, public.currency_code, date, date, date, text, uuid, integer
);

DROP FUNCTION app_private.get_owner_profit_loss_events_page(
  uuid, uuid, public.currency_code, date, date, date, text, uuid, integer
);

CREATE FUNCTION app_private.get_owner_profit_loss_events_page(
...
AS $$
  WITH recognized_events AS (
    SELECT
      'owner_profit_loss_events.v2'::text AS contract_version,
      ...
      invoice.lease_id,
      line.recognized_on,
      pg_catalog.date_trunc('month', line.recognized_on)::date AS period_start,
      line.currency,
      ...
      coalesce(
        ...
        nullif(pg_catalog.btrim(
          coalesce(original_line.customer_label, line.customer_label)
        ), ''),
        pg_catalog.initcap(pg_catalog.replace(
          coalesce(original_line.line_type, line.line_type),
          '_',
          ' '
        ))
      ) AS category_label,
      coalesce(
        ...
        CASE coalesce(original_line.line_type, line.line_type)
          WHEN 'rent' THEN 'rent'
          WHEN 'utility' THEN 'utility_reimbursement'
    ...
    LEFT JOIN public.tenant_invoice_lines AS original_line
      ON original_line.organization_id = line.organization_id
     AND original_line.id = line.reversal_of_id
    LEFT JOIN LATERAL (
      SELECT catalog.id, catalog.code, catalog.display_label,
        catalog.reporting_group
      FROM public.finance_categories AS catalog
      WHERE catalog.organization_id = line.organization_id
        AND catalog.namespace = 'tenant_billing'
        AND (
          catalog.id = coalesce(
            line.finance_category_id,
            original_line.finance_category_id
          )
          OR (
            coalesce(
              line.finance_category_id,
            ...
            AND catalog.code =
              app_private.normalize_finance_category_legacy_code(
                coalesce(original_line.line_type, line.line_type)
              )
          )
        )
    ...
    SELECT
      'owner_profit_loss_events.v2'::text,
      ...
      fee.lease_id,
      fee.fee_date,
      pg_catalog.date_trunc('month', fee.fee_date)::date,
      fee.currency,
      fee.amount,
      'owner_expense'::text,
      'management_fee'::text,
    ...
    SELECT
      ...
      NULL::uuid,
      line.recognized_on,
      pg_catalog.date_trunc('month', line.recognized_on)::date,
      invoice.currency,
      line.amount,
      'owner_expense'::text,
      coalesce(category.code, responsibility.customer_category),
      category.id,
      coalesce(
        category.display_label,
      ...
      coalesce(
        category.reporting_group,
        CASE app_private.normalize_finance_category_legacy_code(
          responsibility.customer_category
        )
    ...
    LEFT JOIN public.finance_categories AS category
      ON category.organization_id = responsibility.organization_id
    ...
    SELECT
      ...
      pg_catalog.date_trunc('month', adjustment.adjustment_date)::date,
      adjustment.currency,
      adjustment.amount,
      'owner_expense'::text,
      coalesce(category.code, responsibility.customer_category),
  ...
  SELECT
    ...
    event.category_reporting_group,
    event.description,
...
CREATE FUNCTION public.get_owner_profit_loss_events_page(
  p_organization_id uuid,
  p_property_id uuid,
  p_currency public.currency_code,
  p_period_start date,
...
COMMENT ON FUNCTION public.get_owner_profit_loss_events_page(
  ...
  'Returns owner-recognized income and expense obligations with stable Finance category identity, current labels, and reporting groups. Tenant receipts, payment allocations, owner balance settlements, Ledger rows, and tenant-recharge company costs are excluded.';
