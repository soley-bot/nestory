-- Integrated from PR113 b4920a9d4fbd5d137872ad67d93c2ab06adf1039 onto 2cfebd6f.
-- New forward identity; preserve existing account attribution and fail closed on
-- excess tenant cash until a canonical tenant-credit liability projection exists.
...
-- Preserve the originating account even if its source link later moves.
CREATE FUNCTION app_private.historical_rent_payment_account(
  p_organization_id uuid, p_payment_id uuid
) RETURNS uuid LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  WITH payment AS (
    SELECT * FROM public.tenant_invoice_payments
    WHERE organization_id=p_organization_id AND id=p_payment_id
  ), bound AS (
    SELECT binding.primary_account_id AS account_id
    FROM app_private.finance_chart_workflow_idempotency_bindings binding
    JOIN app_private.financial_idempotency_requests request
      USING (organization_id,operation,idempotency_key)
    JOIN payment ON request.result_ids->>'paymentId'=payment.id::text
    WHERE binding.organization_id=p_organization_id
      AND binding.operation='record_tenant_invoice_payment' AND request.status='completed'
  ), candidates AS (
    SELECT account_id FROM bound
    UNION ALL
    SELECT history.account_id FROM payment
    JOIN app_private.finance_account_activity_authority_history history
      ON history.organization_id=payment.organization_id
      AND history.authority_kind='source'
      AND history.authority_id=payment.reconciliation_source_id::text
      AND history.valid_from<=payment.created_at
      AND (history.valid_to IS NULL OR payment.created_at<history.valid_to)
    WHERE NOT EXISTS (SELECT 1 FROM bound)
  )
  SELECT CASE WHEN count(DISTINCT account_id)=1
    THEN (array_agg(DISTINCT account_id))[1] END FROM candidates;
$$;
ALTER FUNCTION app_private.historical_rent_payment_account(uuid,uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION app_private.historical_rent_payment_account(uuid,uuid)
  FROM PUBLIC,anon,authenticated,service_role;
...
BEGIN
  IF p_organization_id IS NULL
    ...
    OR p_corrected_due_day IS NULL
  ...
  FROM (
    ...
    UNION ALL
    SELECT 'historical_rent_owner_roster_changed',
      jsonb_build_object('allocationSetId',allocation_set.id)
    FROM public.owner_event_allocation_sets allocation_set
    JOIN public.tenant_invoice_payment_allocations allocation
      ON allocation.organization_id=allocation_set.organization_id
      AND allocation.id=allocation_set.source_line_id
    JOIN public.tenant_invoice_payments payment
      ON payment.organization_id=allocation.organization_id
      AND payment.id=allocation.payment_id
    WHERE allocation_set.organization_id=p_organization_id
      AND allocation_set.source_type='tenant_rent_receipt'
      AND allocation_set.explicit_owner_person_id IS NULL
      AND payment.invoice_id=p_invoice_id AND payment.reversal_of_id IS NULL
      AND NOT EXISTS (SELECT 1 FROM public.tenant_invoice_payments reversal
        WHERE reversal.organization_id=payment.organization_id AND reversal.reversal_of_id=payment.id)
      AND (SELECT jsonb_agg(jsonb_build_array(owner.owner_person_id,owner.ownership_percent_snapshot)
        ORDER BY owner.owner_person_id) FROM public.owner_event_owner_allocations owner
        WHERE owner.organization_id=p_organization_id AND owner.allocation_set_id=allocation_set.id)
      IS DISTINCT FROM (SELECT jsonb_agg(jsonb_build_array(roster.owner_person_id,roster.ownership_percent)
        ORDER BY roster.owner_person_id) FROM app_private.validate_owner_roster_on_date(
          p_organization_id,v_invoice.property_id,v_business_date) roster)

    UNION ALL
    SELECT 'historical_rent_tenant_credit_unsupported',
      jsonb_build_object('creditAmount',
        v_ips_rent_settled+v_owner_rent_settled-p_corrected_rent_amount)
    WHERE v_ips_rent_settled+v_owner_rent_settled>p_corrected_rent_amount

    UNION ALL
    SELECT 'historical_rent_payment_account_unavailable',
      jsonb_build_object('paymentId',payment.id)
    FROM public.tenant_invoice_payments payment
    LEFT JOIN public.financial_reconciliation_sources source
      ON source.organization_id=payment.organization_id
      AND source.id=payment.reconciliation_source_id
    WHERE payment.organization_id=p_organization_id AND payment.invoice_id=p_invoice_id
      AND payment.reversal_of_id IS NULL
      AND NOT EXISTS (SELECT 1 FROM public.tenant_invoice_payments reversal
        WHERE reversal.organization_id=payment.organization_id AND reversal.reversal_of_id=payment.id)
      AND (source.id IS NULL OR source.archived_at IS NOT NULL
        OR app_private.historical_rent_payment_account(p_organization_id,payment.id) IS NULL)
  ) AS blocker;
  ...
  v_preview := pg_catalog.jsonb_build_object(
    ...
    'blockers', v_blockers,
    -- Identities and exact immutable replay evidence are part of confirmation,
    -- so an equal-value replacement receipt cannot reuse an older preview.
    'settlementEvidence', jsonb_build_object(
      'payments', (SELECT coalesce(jsonb_agg(to_jsonb(row) ORDER BY row.id),'[]'::jsonb)
        FROM public.tenant_invoice_payments row
        WHERE row.organization_id=p_organization_id AND row.invoice_id=p_invoice_id),
      'paymentAllocations', (SELECT coalesce(jsonb_agg(to_jsonb(row) ORDER BY row.id),'[]'::jsonb)
        FROM public.tenant_invoice_payment_allocations row
        WHERE row.organization_id=p_organization_id AND row.invoice_id=p_invoice_id),
      'confirmations', (SELECT coalesce(jsonb_agg(to_jsonb(row) ORDER BY row.id),'[]'::jsonb)
        FROM public.owner_collection_confirmations row
        WHERE row.organization_id=p_organization_id AND row.invoice_id=p_invoice_id),
      'confirmationAllocations', (SELECT coalesce(jsonb_agg(to_jsonb(row) ORDER BY row.id),'[]'::jsonb)
        FROM public.owner_collection_confirmation_allocations row
        WHERE row.organization_id=p_organization_id AND row.invoice_id=p_invoice_id),
      'paymentAccounts', (SELECT coalesce(jsonb_agg(jsonb_build_object(
        'paymentId',row.id,'accountId',app_private.historical_rent_payment_account(p_organization_id,row.id)
        ) ORDER BY row.id),'[]'::jsonb)
        FROM public.tenant_invoice_payments row
        WHERE row.organization_id=p_organization_id AND row.invoice_id=p_invoice_id),
      'ownerRoster', (SELECT coalesce(jsonb_agg(to_jsonb(row) ORDER BY row.id),'[]'::jsonb)
        FROM public.property_owners row
        WHERE row.organization_id=p_organization_id AND row.property_id=v_invoice.property_id),
      'ownerCloses', (SELECT coalesce(jsonb_agg(to_jsonb(row) ORDER BY row.id),'[]'::jsonb)
        FROM public.owner_close_series row
        WHERE row.organization_id=p_organization_id AND row.property_id=v_invoice.property_id),
      'fee',to_jsonb(v_fee)
    ),
...
-- Historical successors retain original economic account authority.
  ...
  WITH authorized_account AS (
    ...
    JOIN LATERAL (
      ...
      WHERE candidate.organization_id = line.organization_id
        ...
        AND candidate.valid_from <= coalesce((SELECT original.created_at FROM public.tenant_invoice_lines original WHERE original.organization_id=line.organization_id AND original.id=line.supersedes_line_id),line.created_at)
        AND (candidate.valid_to IS NULL OR coalesce((SELECT original.created_at FROM public.tenant_invoice_lines original WHERE original.organization_id=line.organization_id AND original.id=line.supersedes_line_id),line.created_at) < candidate.valid_to)
    ...
    JOIN LATERAL (
      ...
      WHERE candidate.organization_id = fee.organization_id
        ...
        AND candidate.valid_from <= coalesce((SELECT original.created_at FROM public.management_fee_occurrences original WHERE original.organization_id=fee.organization_id AND original.id=fee.supersedes_occurrence_id),fee.created_at)
        AND (candidate.valid_to IS NULL OR coalesce((SELECT original.created_at FROM public.management_fee_occurrences original WHERE original.organization_id=fee.organization_id AND original.id=fee.supersedes_occurrence_id),fee.created_at) < candidate.valid_to)
      ORDER BY candidate.valid_from DESC LIMIT 1
    ) AS history ON true
