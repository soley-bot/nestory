-- Explain the existing owner-cash reversal guard before confirmation.
-- No consumer evidence is rewritten and no guard authority is weakened.
...
BEGIN
  ...
  FROM (
    ...
    UNION ALL
    SELECT 'historical_rent_dependent_owner_cash',
      jsonb_build_object('paymentId',payment.id,'consumerSourceType',consumer_set.source_type,
        'consumerSourceId',consumer_set.source_id,'consumerSourceLineId',consumer_set.source_line_id,
        'consumedAmount',consumption.consumed_amount)
    FROM public.tenant_invoice_payment_allocations payment_allocation
    JOIN public.tenant_invoice_payments payment
      ON payment.organization_id=payment_allocation.organization_id
      AND payment.id=payment_allocation.payment_id
    JOIN public.owner_event_allocation_sets allocation_set
    ...
    JOIN public.owner_event_owner_allocations owner_allocation
    ...
    JOIN public.owner_component_movements source_movement
    ...
    JOIN public.owner_event_owner_allocations consumer_owner
    ...
    JOIN public.owner_event_allocation_sets consumer_set
    ...
    WHERE payment.organization_id=p_organization_id AND payment.invoice_id=p_invoice_id
      ...
      AND NOT EXISTS (SELECT 1 FROM public.tenant_invoice_payments reversal
        WHERE reversal.organization_id=payment.organization_id AND reversal.reversal_of_id=payment.id)
      AND source_movement.component='ips_held_owner_cash' AND source_movement.signed_amount>0
      AND NOT EXISTS (SELECT 1 FROM public.owner_component_movements consumer_reversal
        WHERE consumer_reversal.organization_id=consumer_movement.organization_id
          AND consumer_reversal.reversal_of_movement_id=consumer_movement.id)
      -- Only this invoice's own fee cash allocations are already reversed by
      -- correct_historical_rent before it invokes the canonical receipt guard.
      AND NOT (consumer_set.source_type='owner_invoice_payment' AND EXISTS (
        SELECT 1 FROM public.owner_charge_cash_allocations cash
        JOIN public.owner_invoice_lines owner_line
          ON owner_line.organization_id=cash.organization_id AND owner_line.id=cash.owner_invoice_line_id
        WHERE cash.organization_id=p_organization_id AND cash.id=consumer_set.source_line_id
          AND cash.reversal_of_id IS NULL AND owner_line.source_type='management_fee'
          AND owner_line.source_id=v_fee.id
      ))
