-- Dedicated atomic historical recovery. Existing single-record corrections
-- preserve downstream reservations, so a later fee can retain August cash.
-- Reverse the fee, correct the payout, then settle the fee using remaining
-- real sources. All three commands share one transaction and preview rollback.
CREATE FUNCTION app_private.run_owner_distribution_fee_recovery(
  ...
  p_withdrawal_id uuid,p_distribution_date date
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
DECLARE
  v_withdrawal public.property_withdrawals%ROWTYPE; v_payout_locked jsonb;
...
BEGIN
  ...
  SELECT * INTO v_withdrawal FROM public.property_withdrawals
    WHERE organization_id=p_organization_id AND id=p_withdrawal_id;
  IF v_withdrawal.id IS NULL OR v_withdrawal.property_id<>v_original.property_id
    OR v_withdrawal.owner_person_id<>v_invoice.owner_person_id OR v_withdrawal.currency<>v_invoice.currency
    OR p_distribution_date IS NULL OR p_distribution_date IN ('infinity'::date,'-infinity'::date)
    OR date_trunc('month',p_distribution_date)<>date_trunc('month',v_original.allocation_date)
    OR p_distribution_date<v_original.allocation_date OR p_distribution_date>=p_new_date
    OR p_distribution_date>=v_withdrawal.withdrawal_date THEN
    RAISE EXCEPTION 'owner_distribution_fee_recovery_scope_invalid' USING ERRCODE='22023';
  END IF;
  PERFORM app_private.assert_property_permission(p_organization_id,v_original.property_id,'finance.correct_records');
  v_payload := app_private.canonical_financial_payload_hash(jsonb_build_object('allocation',p_allocation_id,'date',p_new_date,'reason',btrim(p_reason),'preview',p_preview_hash,'withdrawal',p_withdrawal_id,'distributionDate',p_distribution_date));
  ...
  SELECT to_jsonb(w) INTO v_payout_locked FROM public.property_withdrawals w
    WHERE organization_id=p_organization_id AND id=p_withdrawal_id FOR UPDATE;
  IF v_payout_locked IS DISTINCT FROM to_jsonb(v_withdrawal) THEN
    RAISE EXCEPTION 'fee_payment_date_sources_changed' USING ERRCODE='40001';
  END IF;
  v_hash := app_private.canonical_financial_payload_hash(jsonb_build_object('allocation',p_allocation_id,'newDate',p_new_date,'snapshot',v_locked,'businessDate',v_business_date,'withdrawal',v_payout_locked,'distributionDate',p_distribution_date));
  ...
  v_result := jsonb_build_object('allocationId',p_allocation_id,'oldDate',v_original.allocation_date,'newDate',p_new_date,
    ...
    'currentBalanceChange',0,'withdrawalId',p_withdrawal_id,
    'oldDistributionDate',v_withdrawal.withdrawal_date,'newDistributionDate',p_distribution_date,
    'distributionAmount',v_withdrawal.amount);
  ...
  BEGIN
    ...
    -- The existing payout correction releases its original reservations and
    -- replaces the payout while the fee's original reservation is released.
    -- Its unchanged allocator can use only real cash dated by the payout day.
    -- The replacement fee then consumes the remaining later real receipt.
    v_payout_result := public.correct_owner_distribution_date(p_organization_id,p_withdrawal_id,
      p_distribution_date,CASE WHEN p_preview THEN 'Preview historical fee recovery' ELSE btrim(p_reason) END,
      'fee-recovery-payout:'||v_id::text);
    ...
    VALUES(p_organization_id,v_actor,'fee_payment_date_correction',v_id,'corrected',jsonb_build_object(
      ...
      'newDate',p_new_date,'amount',v_original.amount,'reason',p_reason,'distributionCorrection',v_payout_result));
...
CREATE FUNCTION public.preview_owner_distribution_fee_recovery(
  p_organization_id uuid,p_allocation_id uuid,p_payment_date date,
  p_withdrawal_id uuid,p_distribution_date date
) RETURNS jsonb LANGUAGE sql SECURITY DEFINER SET search_path TO '' AS $$
  SELECT app_private.run_owner_distribution_fee_recovery(p_organization_id,p_allocation_id,p_payment_date,
    'Preview validation only',NULL,NULL,true,p_withdrawal_id,p_distribution_date);
$$;
CREATE FUNCTION public.recover_owner_distribution_fee_dates(
  p_organization_id uuid,p_allocation_id uuid,p_payment_date date,
  p_withdrawal_id uuid,p_distribution_date date,p_reason text,p_preview_hash text,p_idempotency_key text
) RETURNS jsonb LANGUAGE sql SECURITY DEFINER SET search_path TO '' AS $$
  SELECT app_private.run_owner_distribution_fee_recovery(p_organization_id,p_allocation_id,p_payment_date,
    p_reason,p_preview_hash,p_idempotency_key,false,p_withdrawal_id,p_distribution_date);
$$;
REVOKE ALL ON FUNCTION app_private.run_owner_distribution_fee_recovery(uuid,uuid,date,text,text,text,boolean,uuid,date) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.preview_owner_distribution_fee_recovery(uuid,uuid,date,uuid,date) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.recover_owner_distribution_fee_dates(uuid,uuid,date,uuid,date,text,text,text) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.preview_owner_distribution_fee_recovery(uuid,uuid,date,uuid,date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.recover_owner_distribution_fee_dates(uuid,uuid,date,uuid,date,text,text,text) TO authenticated;
