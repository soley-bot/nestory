...
ALTER TABLE public.lease_deposit_events
  ADD CONSTRAINT lease_deposit_events_ledger_entry_fkey
    FOREIGN KEY (ledger_entry_id)
    REFERENCES public.ledger_entries(id)
    ON DELETE RESTRICT,
  ADD CONSTRAINT lease_deposit_events_ledger_entry_unique
    UNIQUE (ledger_entry_id);

ALTER FUNCTION public.record_lease_deposit_event(
  uuid, uuid, text, date, numeric, text
)
RENAME TO record_lease_deposit_event_operational_unchecked;

ALTER FUNCTION public.reverse_lease_deposit_event(
  uuid, uuid, date, text
)
RENAME TO reverse_lease_deposit_event_operational_unchecked;

REVOKE ALL ON FUNCTION public.record_lease_deposit_event_operational_unchecked(
  uuid, uuid, text, date, numeric, text
) FROM PUBLIC, anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.reverse_lease_deposit_event_operational_unchecked(
  uuid, uuid, date, text
) FROM PUBLIC, anon, authenticated, service_role;
...
BEGIN
  ...
  v_event_id := public.record_lease_deposit_event_operational_unchecked(
    p_organization_id,
    p_lease_deposit_id,
    p_event_type,
    p_event_date,
...
BEGIN
  ...
  v_reversal_id := public.reverse_lease_deposit_event_operational_unchecked(
    p_organization_id,
    p_event_id,
    p_event_date,
    p_reference
