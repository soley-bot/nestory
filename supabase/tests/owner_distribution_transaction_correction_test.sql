...
SELECT throws_ok($$SELECT public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',
 (original->>'property_withdrawal_id')::uuid,current_date-61,400,'Original payout','Correct actual bank date','date-test-unfunded')
...
UPDATE owner_date_state SET corrected=public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',
 (original->>'property_withdrawal_id')::uuid,(date_trunc('month',current_date)-interval '1 day')::date,400,'Original payout',
...
SELECT is((SELECT public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',
 (original->>'property_withdrawal_id')::uuid,(date_trunc('month',current_date)-interval '1 day')::date,400,'Original payout',
...
SELECT throws_ok($$SELECT public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',
 (original->>'property_withdrawal_id')::uuid,current_date-10,400,'Original payout',
 'Correct actual bank date','date-test-correct') FROM owner_date_state$$,
 NULL,NULL,'same key cannot be reused with a different payload');
SELECT throws_ok($$SELECT public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',
 (original->>'property_withdrawal_id')::uuid,current_date-10,400,'Original payout','Different date request','date-test-second') FROM owner_date_state$$,
...
SELECT throws_ok($$SELECT public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',
 (later->>'property_withdrawal_id')::uuid,current_date+1,100,'Later payout','Correct actual bank date','date-test-future') FROM owner_date_state$$,
 '22023','owner_distribution_date_in_future','future date is rejected');

-- Expanded transaction corrections preserve the same historical cash authority.
SELECT throws_ok($$SELECT public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',(corrected->>'withdrawalId')::uuid,(corrected->>'newDate')::date,400,'Original payout','Correct bank statement','same-input') FROM owner_date_state$$,'22023',NULL,'unchanged transaction rejected');
SELECT throws_ok($$SELECT public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',(corrected->>'withdrawalId')::uuid,(corrected->>'newDate')::date,0,'Original payout','Correct bank statement','zero-amount') FROM owner_date_state$$,'22023',NULL,'zero amount rejected');
SELECT throws_ok($$SELECT public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',(corrected->>'withdrawalId')::uuid,(corrected->>'newDate')::date,1.001,'Original payout','Correct bank statement','fraction-amount') FROM owner_date_state$$,'22023',NULL,'fractional cents rejected');
SELECT throws_ok($$SELECT public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',(corrected->>'withdrawalId')::uuid,(corrected->>'newDate')::date,10000,'Original payout','Correct bank statement','overfunded-amount') FROM owner_date_state$$,'23514',NULL,'increase exceeding reserved historical cash rejected');
UPDATE owner_date_state SET corrected=public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',(corrected->>'withdrawalId')::uuid,(corrected->>'newDate')::date,450,'Higher payout','Correct bank statement','increase-amount');
SELECT is((SELECT sum(signed_amount) FROM public.owner_component_movements WHERE property_id='dd150000-0000-4000-8000-000000000002' AND component='ips_held_owner_cash'),350::numeric,'increased payout reduces available cash by exactly difference');
UPDATE owner_date_state SET corrected=public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',(corrected->>'withdrawalId')::uuid,(corrected->>'newDate')::date,350,'Lower payout','Correct bank statement','decrease-amount');
SELECT is((SELECT sum(signed_amount) FROM public.owner_component_movements WHERE property_id='dd150000-0000-4000-8000-000000000002' AND component='ips_held_owner_cash'),450::numeric,'reduced payout releases exactly the difference');
UPDATE owner_date_state SET corrected=public.correct_owner_distribution('dd150000-0000-4000-8000-000000000001',(corrected->>'withdrawalId')::uuid,(corrected->>'newDate')::date,350,'Bank transfer corrected','Correct bank statement','reference-only');
SELECT is((SELECT reference FROM public.property_withdrawals WHERE id=(SELECT (corrected->>'withdrawalId')::uuid FROM owner_date_state)),'Bank transfer corrected','reference-only correction stores owner-facing reference separately from audit reason');
...
SELECT is((SELECT count(*) FROM public.activity_logs WHERE organization_id='dd150000-0000-4000-8000-000000000001'
 AND entity_type='owner_distribution_correction'),5::bigint,'each successful correction records one audit entry');
SELECT * FROM finish();
ROLLBACK;
