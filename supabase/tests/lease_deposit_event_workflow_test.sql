...
SELECT ok(
  NOT has_function_privilege(
    'authenticated',
    'app_private.record_lease_deposit_event(uuid,uuid,text,date,numeric,text)',
    'EXECUTE'
  ),
  'authenticated cannot execute the private deposit writer'
);
SELECT ok(NOT has_function_privilege('authenticated','public.record_lease_deposit_event(uuid,uuid,text,date,numeric,text)','EXECUTE') AND has_function_privilege('authenticated','public.record_lease_deposit_event_with_account(uuid,uuid,uuid,text,date,numeric,text)','EXECUTE') AND has_function_privilege('authenticated','public.reverse_lease_deposit_event(uuid,uuid,date,text)','EXECUTE'),'authenticated can execute only the account-aware deposit writer and checked reversal');
